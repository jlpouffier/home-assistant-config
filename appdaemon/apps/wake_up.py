...
class wake_up(hass.Hass):
  def initialize(self):
    ...
    self.log("Waking-up Automations initialized", log = 'user_log')
  ...
  def callback_schedule_wake_up(self, kwargs):
    ...
    if self.get_state("binary_sensor.workday_today") == "on" and self.get_state("binary_sensor.home_occupied") == "on":
      ...
      if wake_up_datetime > self.datetime() + datetime.timedelta(minutes = 1) :
          self.log("Wake up automation will be turned on at :", log = 'user_log')
          self.log(wake_up_datetime, log = 'user_log')
          # ... and register the wake-up callback
          self.run_at(self.callback_wake_up, wake_up_datetime)
      else: 
          self.log("Wake up automation won't be turned on today because wake-up time is in the past", log = 'user_log')
          self.log(wake_up_datetime, log = 'user_log')
      ...
      if coffee_maker_turn_on_time > self.datetime() + datetime.timedelta(minutes = 1) :
          self.log("Coffee maker will be turned on at :", log = 'user_log')
          self.log(coffee_maker_turn_on_time, log = 'user_log')
          # ... and register the wake-up callback
          self.run_at(self.callback_turn_on_coffee_maker, coffee_maker_turn_on_time)
      else:
          self.log("Coffee maker won't be turned on today because <coffee maker turn on> time is in the past :", log = 'user_log')
          self.log(coffee_maker_turn_on_time, log = 'user_log')
  ...
  def callback_wake_up(self, kwargs):
    ...
    self.log("Wake up automation !", log = 'user_log')
  ...
  def callback_turn_on_coffee_maker(self, kwargs):
    self.log("Turning on coffee maker !", log = 'user_log')
  ...
  def callback_jl_phone_alarm_changed(self, entity, attribute, old, new, kwargs):
    if new != "unavailable":
      ...
      if jl_home_alarm_time != current_wake_up_time:
        ...
        self.log("Phone alarmed different than Wake-up time. Notifying it ...", log = 'user_log')
  ...
  def callback_button_clicked_set_new_wake_up_time(self, event_name, data, kwargs):
    ...
    self.log("Notification button clicked : Setting wake_up_time to JL's phone alarm time ... ", log = 'user_log') 
